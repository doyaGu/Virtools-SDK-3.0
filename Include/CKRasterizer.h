...
//--- Implemented by lib : keep a track of other rasterizers
	void	LinkRasterizer(CKRasterizer* rst) { if (rst!=this) m_OtherRasterizers.PushBack(rst); }
	void	RemoveLinkedRasterizer(CKRasterizer* rst) { if (rst!=this) m_OtherRasterizers.Remove(rst); }
...
//--- Lighting & Material States
	...
	virtual BOOL SetMaterial(CKMaterialData* mat) { if (mat) m_CurrentMaterialData = *mat; return FALSE; }
...
//--- Try to set a texture as the target for rendering
	virtual BOOL SetTargetTexture(CKDWORD TextureObject,int Width = 0, int Height = 0,CKRST_CUBEFACE Face = CKRST_CUBEFACE_XPOS ) { return FALSE; }
...
// for texture and render states
	...
	CKRenderStateData				m_TexStateCache[CKRST_MAX_STAGES][CKRST_TSS_MAXSTATE];	
	...
	class CKRstCriticalSection {
		public:
			CKRstCriticalSection(CKRasterizer* Rst):m_Rst(Rst) {
				if (Rst && Rst->m_Mutex) Rst->m_Mutex->EnterMutex();
			}
			~CKRstCriticalSection() {
				if (m_Rst && m_Rst->m_Mutex) m_Rst->m_Mutex->LeaveMutex();
			}
			CKRasterizer* m_Rst;
		};
...
inline BOOL CKRasterizerContext::InternalSetRenderState(VXRENDERSTATETYPE State,CKDWORD Value) {
	if (m_StateCache[State].Flags & (RSC_LOCKED|RSC_DISABLED)) return TRUE;
...
inline BOOL CKRasterizerContext::InternalSetTextureState(int Stage,CKRST_TEXTURESTAGESTATETYPE State,CKDWORD Value) {
	if (m_TexStateCache[Stage][State].Flags & (RSC_LOCKED|RSC_DISABLED)) return TRUE;
...
inline  BOOL CKRasterizerContext::InternalGetTextureState(int Stage,CKRST_TEXTURESTAGESTATETYPE State,CKDWORD* Value) {
	if (m_TexStateCache[Stage][State].Flags & RSC_VALID) {
		*Value = m_TexStateCache[Stage][State].Value;			return TRUE; 
	} else {
		*Value = m_TexStateCache[Stage][State].DefaultValue;	return FALSE;
	}
}
